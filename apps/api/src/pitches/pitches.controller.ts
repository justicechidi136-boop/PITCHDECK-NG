...
import { createPitchSchema, updatePitchSchema } from "@pitchdeck/contracts";
...
export class PitchesController {
  ...
  @Post()
  create(@CurrentUser() user: RequestUser, @Body(new ZodValidationPipe(createPitchSchema)) body: { title: string }) {
  ...
  update(
    ...
    @Body(new ZodValidationPipe(updatePitchSchema)) body: Record<string, unknown>,
  ) {
    return this.pitchesService.update(user, pitchId, body);
  }

  @Delete(":pitchId")
  delete(@CurrentUser() user: RequestUser, @Param("pitchId", ParseUUIDPipe) pitchId: string) {
    return this.pitchesService.delete(user, pitchId);
  }

  @Get(":pitchId/completeness")
  completeness(@CurrentUser() user: RequestUser, @Param("pitchId", ParseUUIDPipe) pitchId: string) {
    return this.pitchesService.getCompleteness(user, pitchId);
  }

  @Post(":pitchId/submit")
  submit(@CurrentUser() user: RequestUser, @Param("pitchId", ParseUUIDPipe) pitchId: string) {
    return this.pitchesService.submit(user, pitchId);
  }

  @Post(":pitchId/withdraw")
  withdraw(@CurrentUser() user: RequestUser, @Param("pitchId", ParseUUIDPipe) pitchId: string) {
    return this.pitchesService.withdraw(user, pitchId);
  }

  @Post(":pitchId/resubmit")
  resubmit(@CurrentUser() user: RequestUser, @Param("pitchId", ParseUUIDPipe) pitchId: string) {
    return this.pitchesService.resubmit(user, pitchId);
  }

  @Get(":pitchId/submissions")
  listSubmissions(@CurrentUser() user: RequestUser, @Param("pitchId", ParseUUIDPipe) pitchId: string) {
